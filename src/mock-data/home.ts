export const HOME_CONSTANT_DATA: any = {
    data: {
        landings: {
            data: [
                {
                    attributes: {
                        name: "Home Landing",
                        label: "home-landing",
                        banners: {
                            data: [
                                {
                                    attributes: {
                                        name: "Home Banner",
                                        label: "home-banner",
                                        title: "We build digital solutions to achieve your most ambitious goals",
                                        subtitle:
                                            "Nearshore Software Development Company in Latin America​",
                                        description:
                                            "At Novatech we become your strategic, technological partner. We help you stand out from the competition with innovative, creative and outside the box software development solutions.",
                                        isFloating: true,
                                        top: "0",
                                        left: "0",
                                        right: "-13rem",
                                        bottom: "0",
                                        haveAction: true,
                                        actionLabel: "Get a Quote",
                                        actionRoute: "/services",
                                        hero: {
                                            data: {
                                                attributes: {
                                                    name: "homebannerhero.png",
                                                    formats: {
                                                        thumbnail: {
                                                            name: "thumbnail_homebannerhero.png",
                                                            hash: "thumbnail_homebannerhero_a306162f84",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 243,
                                                            height: 156,
                                                            size: 27.56,
                                                            path: null,
                                                            url: "/uploads/thumbnail_homebannerhero_a306162f84.png",
                                                        },
                                                        large: {
                                                            name: "large_homebannerhero.png",
                                                            hash: "large_homebannerhero_a306162f84",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 1000,
                                                            height: 643,
                                                            size: 179.67,
                                                            path: null,
                                                            url: "/uploads/large_homebannerhero_a306162f84.png",
                                                        },
                                                        medium: {
                                                            name: "medium_homebannerhero.png",
                                                            hash: "medium_homebannerhero_a306162f84",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 750,
                                                            height: 482,
                                                            size: 117.45,
                                                            path: null,
                                                            url: "/uploads/medium_homebannerhero_a306162f84.png",
                                                        },
                                                        small: {
                                                            name: "small_homebannerhero.png",
                                                            hash: "small_homebannerhero_a306162f84",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 500,
                                                            height: 321,
                                                            size: 67.69,
                                                            path: null,
                                                            url: "/uploads/small_homebannerhero_a306162f84.png",
                                                        },
                                                    },
                                                },
                                            },
                                        },
                                        background: {
                                            data: {
                                                attributes: {
                                                    name: "homebannerbg.png",
                                                    formats: {
                                                        thumbnail: {
                                                            name: "thumbnail_homebannerbg.png",
                                                            hash: "thumbnail_homebannerbg_4ae40ba030",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 245,
                                                            height: 131,
                                                            size: 27.38,
                                                            path: null,
                                                            url: "/uploads/thumbnail_homebannerbg_4ae40ba030.png",
                                                        },
                                                        large: {
                                                            name: "large_homebannerbg.png",
                                                            hash: "large_homebannerbg_4ae40ba030",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 1000,
                                                            height: 533,
                                                            size: 272.47,
                                                            path: null,
                                                            url: "/uploads/large_homebannerbg_4ae40ba030.png",
                                                        },
                                                        medium: {
                                                            name: "medium_homebannerbg.png",
                                                            hash: "medium_homebannerbg_4ae40ba030",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 750,
                                                            height: 400,
                                                            size: 171.94,
                                                            path: null,
                                                            url: "/uploads/medium_homebannerbg_4ae40ba030.png",
                                                        },
                                                        small: {
                                                            name: "small_homebannerbg.png",
                                                            hash: "small_homebannerbg_4ae40ba030",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 500,
                                                            height: 267,
                                                            size: 88.83,
                                                            path: null,
                                                            url: "/uploads/small_homebannerbg_4ae40ba030.png",
                                                        },
                                                    },
                                                },
                                            },
                                        },
                                    },
                                },
                                {
                                    attributes: {
                                        name: "Our work banner",
                                        label: "our-work-banner",
                                        title: "We develop unique digital experiences​",
                                        subtitle: "Our work",
                                        description:
                                            "Novatech continues to be trusted by leading multinational companies in the U.S. and Latin America to assist in the digital transformation of their businesses.",
                                        isFloating: false,
                                        top: null,
                                        left: null,
                                        right: null,
                                        bottom: null,
                                        haveAction: true,
                                        actionLabel: "Read Case Studies​",
                                        actionRoute: "/services",
                                        hero: {
                                            data: {
                                                attributes: {
                                                    name: "phone.png",
                                                    formats: {
                                                        thumbnail: {
                                                            name: "thumbnail_phone.png",
                                                            hash: "thumbnail_phone_6d0f7fb783",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 146,
                                                            height: 156,
                                                            size: 30.7,
                                                            path: null,
                                                            url: "/uploads/thumbnail_phone_6d0f7fb783.png",
                                                        },
                                                        large: {
                                                            name: "large_phone.png",
                                                            hash: "large_phone_6d0f7fb783",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 938,
                                                            height: 1000,
                                                            size: 623.86,
                                                            path: null,
                                                            url: "/uploads/large_phone_6d0f7fb783.png",
                                                        },
                                                        medium: {
                                                            name: "medium_phone.png",
                                                            hash: "medium_phone_6d0f7fb783",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 703,
                                                            height: 750,
                                                            size: 378.65,
                                                            path: null,
                                                            url: "/uploads/medium_phone_6d0f7fb783.png",
                                                        },
                                                        small: {
                                                            name: "small_phone.png",
                                                            hash: "small_phone_6d0f7fb783",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 469,
                                                            height: 500,
                                                            size: 189.97,
                                                            path: null,
                                                            url: "/uploads/small_phone_6d0f7fb783.png",
                                                        },
                                                    },
                                                },
                                            },
                                        },
                                        background: {
                                            data: {
                                                attributes: {
                                                    name: "bg_our work.png",
                                                    formats: {
                                                        thumbnail: {
                                                            name: "thumbnail_bg_our work.png",
                                                            hash: "thumbnail_bg_our_work_d6e85f3964",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 183,
                                                            height: 156,
                                                            size: 9.4,
                                                            path: null,
                                                            url: "/uploads/thumbnail_bg_our_work_d6e85f3964.png",
                                                        },
                                                        large: {
                                                            name: "large_bg_our work.png",
                                                            hash: "large_bg_our_work_d6e85f3964",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 1000,
                                                            height: 854,
                                                            size: 82.04,
                                                            path: null,
                                                            url: "/uploads/large_bg_our_work_d6e85f3964.png",
                                                        },
                                                        medium: {
                                                            name: "medium_bg_our work.png",
                                                            hash: "medium_bg_our_work_d6e85f3964",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 750,
                                                            height: 640,
                                                            size: 55.87,
                                                            path: null,
                                                            url: "/uploads/medium_bg_our_work_d6e85f3964.png",
                                                        },
                                                        small: {
                                                            name: "small_bg_our work.png",
                                                            hash: "small_bg_our_work_d6e85f3964",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 500,
                                                            height: 427,
                                                            size: 33.21,
                                                            path: null,
                                                            url: "/uploads/small_bg_our_work_d6e85f3964.png",
                                                        },
                                                    },
                                                },
                                            },
                                        },
                                    },
                                },
                                {
                                    attributes: {
                                        name: "Services Banner",
                                        label: "services-banner",
                                        title: "Careers",
                                        subtitle:
                                            "At Novatech, we look to attract and retain the brightest minds in the business, hiring professionals and students in every area of the firm. Our people are the core of our business​!",
                                        description: "",
                                        isFloating: false,
                                        top: null,
                                        left: null,
                                        right: null,
                                        bottom: null,
                                        haveAction: true,
                                        actionLabel:
                                            "Explore Your Opportunities​",
                                        actionRoute: "/services",
                                        hero: {
                                            data: null,
                                        },
                                        background: {
                                            data: {
                                                attributes: {
                                                    name: "coworkers-team-working-brainstorming-concept 2.png",
                                                    formats: {
                                                        thumbnail: {
                                                            name: "thumbnail_coworkers-team-working-brainstorming-concept 2.png",
                                                            hash: "thumbnail_coworkers_team_working_brainstorming_concept_2_e057871f4b",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 245,
                                                            height: 99,
                                                            size: 64.59,
                                                            path: null,
                                                            url: "/uploads/thumbnail_coworkers_team_working_brainstorming_concept_2_e057871f4b.png",
                                                        },
                                                        large: {
                                                            name: "large_coworkers-team-working-brainstorming-concept 2.png",
                                                            hash: "large_coworkers_team_working_brainstorming_concept_2_e057871f4b",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 1000,
                                                            height: 404,
                                                            size: 897.18,
                                                            path: null,
                                                            url: "/uploads/large_coworkers_team_working_brainstorming_concept_2_e057871f4b.png",
                                                        },
                                                        medium: {
                                                            name: "medium_coworkers-team-working-brainstorming-concept 2.png",
                                                            hash: "medium_coworkers_team_working_brainstorming_concept_2_e057871f4b",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 750,
                                                            height: 303,
                                                            size: 520.47,
                                                            path: null,
                                                            url: "/uploads/medium_coworkers_team_working_brainstorming_concept_2_e057871f4b.png",
                                                        },
                                                        small: {
                                                            name: "small_coworkers-team-working-brainstorming-concept 2.png",
                                                            hash: "small_coworkers_team_working_brainstorming_concept_2_e057871f4b",
                                                            ext: ".png",
                                                            mime: "image/png",
                                                            width: 500,
                                                            height: 202,
                                                            size: 243.59,
                                                            path: null,
                                                            url: "/uploads/small_coworkers_team_working_brainstorming_concept_2_e057871f4b.png",
                                                        },
                                                    },
                                                },
                                            },
                                        },
                                    },
                                },
                            ],
                        },
                        tech_carousel: {
                            data: {
                                attributes: {
                                    title: "Our technologies",
                                    teches: {
                                        data: [
                                            {
                                                attributes: {
                                                    name: "AWS",
                                                    label: "aws",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_aws.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_aws.png",
                                                                        hash: "thumbnail_ic_aws_dbc856fe11",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 8.14,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_aws_dbc856fe11.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "Angular",
                                                    label: "angular",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_angular.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_angular.png",
                                                                        hash: "thumbnail_ic_angular_f5411ce3c6",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 4.86,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_angular_f5411ce3c6.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "Docker",
                                                    label: "docker",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_docker.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_docker.png",
                                                                        hash: "thumbnail_ic_docker_dedc32aa15",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 5.81,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_docker_dedc32aa15.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "Laravel",
                                                    label: "laravel",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_laravel.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_laravel.png",
                                                                        hash: "thumbnail_ic_laravel_9ab819fe8f",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 6.19,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_laravel_9ab819fe8f.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "Node JS",
                                                    label: "node-js",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_Nodejs.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_Nodejs.png",
                                                                        hash: "thumbnail_ic_Nodejs_e952784085",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 6.38,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_Nodejs_e952784085.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "React",
                                                    label: "react",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_react.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_react.png",
                                                                        hash: "thumbnail_ic_react_943683d747",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 10.54,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_react_943683d747.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                            {
                                                attributes: {
                                                    name: "Sql",
                                                    label: "sql",
                                                    logo: {
                                                        data: {
                                                            attributes: {
                                                                name: "ic_sql.png",
                                                                formats: {
                                                                    thumbnail: {
                                                                        name: "thumbnail_ic_sql.png",
                                                                        hash: "thumbnail_ic_sql_6cdaee7f02",
                                                                        ext: ".png",
                                                                        mime: "image/png",
                                                                        width: 156,
                                                                        height: 156,
                                                                        size: 2.89,
                                                                        path: null,
                                                                        url: "/uploads/thumbnail_ic_sql_6cdaee7f02.png",
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            },
                                        ],
                                    },
                                },
                            },
                        },
                        info_card_boxes: {
                            data: [
                                {
                                    attributes: {
                                        title: "Nearshore Software development",
                                        subtitle: null,
                                        label: "nearshore-box",
                                        bgColor: "#1F2234",
                                        haveAction: null,
                                        actionLabel: null,
                                        actionRoute: null,
                                        isUppercaseTitle: null,
                                        info_cards: {
                                            data: [
                                                {
                                                    attributes: {
                                                        title: "Nearshore software development",
                                                        subtitle: null,
                                                        isTransparent: true,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Nearshore software development.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Nearshore software development.png",
                                                                                hash: "thumbnail_Nearshore_software_development_6a7131f56f",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 245,
                                                                                height: 107,
                                                                                size: 16.58,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Nearshore_software_development_6a7131f56f.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Location. Location. Location",
                                                        subtitle:
                                                            "Minimize travel time for on-site meetings.",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "location-outline 2.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_location-outline 2.png",
                                                                                hash: "thumbnail_location_outline_2_23b54e977d",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 11.55,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_location_outline_2_23b54e977d.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Cost benefits",
                                                        subtitle:
                                                            "Skilled, affordable talent. Rates are ~50% of a U.S. engineer with similar level of expertise, without compromising effectiveness or job quality.",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "cash-outline 2.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_cash-outline 2.png",
                                                                                hash: "thumbnail_cash_outline_2_1cc136375c",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 10.34,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_cash_outline_2_1cc136375c.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Same time zone enables real time collaboration",
                                                        subtitle:
                                                            "Working on the same time zone allows for increased collaboration between LatAm and U.S. employees.",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "new_earth-outline.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_new_earth-outline.png",
                                                                                hash: "thumbnail_new_earth_outline_20c75c4848",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 18.78,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_new_earth_outline_20c75c4848.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Seamless cultural integration",
                                                        subtitle:
                                                            "Strong English proficiency, good communication skills and cultural alignment (due to an Americanized economy) enables fast integration into onshore operations.",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Puzzle-outline.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Puzzle-outline.png",
                                                                                hash: "thumbnail_Puzzle_outline_bd260184e8",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 12.05,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Puzzle_outline_bd260184e8.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Large talent pool of developers in LatAm",
                                                        subtitle:
                                                            "Access to a vast selection of well-prepared individuals across multiple technologies.",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "new_talent-outline.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_new_talent-outline.png",
                                                                                hash: "thumbnail_new_talent_outline_2d761d1399",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 15.03,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_new_talent_outline_2d761d1399.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            ],
                                        },
                                    },
                                },
                                {
                                    attributes: {
                                        title: "Services",
                                        subtitle:
                                            "We work under and agile methodology based on Staff Augmentation. Our service model includes 3 different options in order to provide the solution that best fits our client’s development needs.​",
                                        label: "services-box",
                                        bgColor: "#FFF",
                                        haveAction: true,
                                        actionLabel: "Read more",
                                        actionRoute: "/services",
                                        isUppercaseTitle: false,
                                        info_cards: {
                                            data: [
                                                {
                                                    attributes: {
                                                        title: "Nova",
                                                        subtitle:
                                                            "Project and development team fully managed by Novatech (in-house-projects)",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Nova.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Nova.png",
                                                                                hash: "thumbnail_Nova_36c8914b44",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 19.95,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Nova_36c8914b44.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Squad",
                                                        subtitle:
                                                            "Outsourcing of a development team to be fully manged by client's Product Owner",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Corp.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Corp.png",
                                                                                hash: "thumbnail_Corp_ce5f57a523",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 14.22,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Corp_ce5f57a523.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        title: "Rocket",
                                                        subtitle:
                                                            "Boost of client's development capacity",
                                                        isTransparent: false,
                                                        icon: {
                                                            data: {
                                                                attributes: {
                                                                    name: "rocket.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_rocket.png",
                                                                                hash: "thumbnail_rocket_4083710e56",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 156,
                                                                                height: 156,
                                                                                size: 12.23,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_rocket_4083710e56.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            ],
                                        },
                                    },
                                },
                            ],
                        },
                        customer_carousels: {
                            data: [
                                {
                                    attributes: {
                                        title: "Our customers",
                                        haveAction: true,
                                        actionLabel: "Check our work",
                                        actionRoute: "/our-work",
                                        customers: {
                                            data: [
                                                {
                                                    attributes: {
                                                        name: "Cuscatlan",
                                                        logo: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Cuscatlan.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Cuscatlan.png",
                                                                                hash: "thumbnail_Cuscatlan_ad43b516ea",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 245,
                                                                                height: 92,
                                                                                size: 18.8,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Cuscatlan_ad43b516ea.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        name: "Banco Atlantida",
                                                        logo: {
                                                            data: {
                                                                attributes: {
                                                                    name: "banco-atlantida-logo-red.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_banco-atlantida-logo-red.png",
                                                                                hash: "thumbnail_banco_atlantida_logo_red_46250995ed",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 245,
                                                                                height: 60,
                                                                                size: 13.99,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_banco_atlantida_logo_red_46250995ed.png",
                                                                            },
                                                                        small: {
                                                                            name: "small_banco-atlantida-logo-red.png",
                                                                            hash: "small_banco_atlantida_logo_red_46250995ed",
                                                                            ext: ".png",
                                                                            mime: "image/png",
                                                                            width: 500,
                                                                            height: 123,
                                                                            size: 38.15,
                                                                            path: null,
                                                                            url: "/uploads/small_banco_atlantida_logo_red_46250995ed.png",
                                                                        },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        name: "Hugo",
                                                        logo: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Hugo.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Hugo.png",
                                                                                hash: "thumbnail_Hugo_7d5081a737",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 168,
                                                                                height: 156,
                                                                                size: 15.31,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Hugo_7d5081a737.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        name: "Bancolombia",
                                                        logo: {
                                                            data: {
                                                                attributes: {
                                                                    name: "Logo_Bancolombia.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_Logo_Bancolombia.png",
                                                                                hash: "thumbnail_Logo_Bancolombia_a40a630ecd",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 245,
                                                                                height: 58,
                                                                                size: 14.89,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_Logo_Bancolombia_a40a630ecd.png",
                                                                            },
                                                                        medium: {
                                                                            name: "medium_Logo_Bancolombia.png",
                                                                            hash: "medium_Logo_Bancolombia_a40a630ecd",
                                                                            ext: ".png",
                                                                            mime: "image/png",
                                                                            width: 750,
                                                                            height: 176,
                                                                            size: 61.35,
                                                                            path: null,
                                                                            url: "/uploads/medium_Logo_Bancolombia_a40a630ecd.png",
                                                                        },
                                                                        small: {
                                                                            name: "small_Logo_Bancolombia.png",
                                                                            hash: "small_Logo_Bancolombia_a40a630ecd",
                                                                            ext: ".png",
                                                                            mime: "image/png",
                                                                            width: 500,
                                                                            height: 118,
                                                                            size: 37.45,
                                                                            path: null,
                                                                            url: "/uploads/small_Logo_Bancolombia_a40a630ecd.png",
                                                                        },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                                {
                                                    attributes: {
                                                        name: "Unicomer",
                                                        logo: {
                                                            data: {
                                                                attributes: {
                                                                    name: "grupo-unicomer.png",
                                                                    formats: {
                                                                        thumbnail:
                                                                            {
                                                                                name: "thumbnail_grupo-unicomer.png",
                                                                                hash: "thumbnail_grupo_unicomer_94bb8a1934",
                                                                                ext: ".png",
                                                                                mime: "image/png",
                                                                                width: 245,
                                                                                height: 110,
                                                                                size: 27.87,
                                                                                path: null,
                                                                                url: "/uploads/thumbnail_grupo_unicomer_94bb8a1934.png",
                                                                            },
                                                                    },
                                                                },
                                                            },
                                                        },
                                                    },
                                                },
                                            ],
                                        },
                                    },
                                },
                            ],
                        },
                    },
                },
            ],
        },
    },
};
